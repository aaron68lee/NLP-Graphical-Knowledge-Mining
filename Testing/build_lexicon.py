...
from Testing import word_lists #Alina wrote, get it here: https://github.com/arsena-k/Word2Vec-bias-extraction
import numpy as np
...
def clean_words(word_list, w2vmodel, returnNA, min_count=1): #by default, doesn't return words not in the vocab
    ...
    for i in word_list:
        if returnNA==False:
            try:
                w2vmodel.wv[i]
                if vocab.count(i) >= min_count: #skip this word if it is not in the model at least min count times
...
def clean_words_pairwise(pair1, pair2, w2vmodel, returnNA, min_count=1): 
    ...
    for a,b in zipped:
        if returnNA==False:
            ...
            if (vocab.count(a) >= min_count and vocab.count(b) >= min_count):
